var GitManualGrammar = function (p) {
	return {
		...
		statement: [
			...
			'$commandName $commandOption will $gitVerbPerform $adjective#prependAn $commandName before [$gitVerb#verbPresentParticiplify the $object|doing anything else]',
			'$multipleObjects are $gitVerb#verbPastTensify to $constantObject by $commandName',
			'the same set of $object#pluralize would [$wordSometimes |]be $gitVerb#verbPastTensify in $adjective#prependAn $object',
			'$multipleObjects that were $wordPreviously $gitVerb#verbPastTensify $preposition the $adjective $object#pluralize are $gitVerb#verbPastTensify to $adjective#prependAn $object',
			'it is [$wordSometimes |]$wordPossible that $gitVerb#verbPastTensify#prependAn $wordFailure will prevent $adjective $gitVerb#verbPresentParticiplify of $multipleObjects',
			'$subject $wordWill $gitVerb any such $wordFailure#pluralize and run $commandName $commandOption instead',
			'to $gitVerb $adjective#prependAn $constantObject and $gitVerb the working $object#pluralize, use the command $commandName $commandOption instead',
			'the $object to be $gitVerb#verbPastTensify can be $wordSupplied in several ways',
			'the $commandOption $wordOption can be used to $gitVerb $object#prependAn for the $object that is $gitVerb#verbPastTensify by $adjective#prependAn $object',
			...
			'$multipleObjects $gitVerb#verbPastTensify by $object#pluralize in the $locatedObject, but that [$wordSometimes |]are [not |]in $constantObject, are $gitVerb#verbPastTensify in $adjective#prependAn $object',
		],
	}
}
